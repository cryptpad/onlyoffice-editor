...
(function (window, undefined) {
	...
	CDocument.prototype.toZip = function () {
		...
		if (jsZipWrapper.create()) {
			...
			docPart.setData(filePart.getXmlBytes(this, memory));
			memory.Seek(0);
			...
			var webSettingsPart = docPart.addPart(AscCommon.openXml.Types.webSettings);
			webSettingsPart.setData(sampleData);
			memory.Seek(0);
			...
			var settingsPart = docPart.addPart(AscCommon.openXml.Types.documentSettings);
			settingsPart.setData(sampleData);
			memory.Seek(0);
			...
			sampleData = memory.GetDataUint8();
			var stylesPart = docPart.addPart(AscCommon.openXml.Types.styles);
			stylesPart.setData(sampleData);
			memory.Seek(0);
			...
			sampleData = memory.GetDataUint8();
			var themePart = docPart.addPart(AscCommon.openXml.Types.theme);
			...
			sampleData = memory.GetDataUint8();
			var fontsPart = docPart.addPart(AscCommon.openXml.Types.fontTable);
			fontsPart.setData(sampleData);
