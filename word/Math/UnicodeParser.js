...
(function (window) {
	...
	CUnicodeParser.prototype.GetExpBracketLiteral = function () {
		...
		return {
			type: oLiteralNames.bracketBlockLiteral[0],
			value: exp,
			open,
			close,
		};
	...
	CUnicodeParser.prototype.GetScriptBelowOrAboveContent = function (base) {
		...
		return {
			type: oLiteralNames.aboveLiteral[num],
			value: base,
			up: oAboveLiteral,
			down: oBelowLiteral
		};
	...
	CUnicodeParser.prototype.GetOperandLiteral = function (isNoSubSup) {
		...
		if (this.IsSpecial(isNoSubSup) && arrFactorList.length === 1) {
			let oContent = arrFactorList[0];
			...
			return this.GetContentOfLiteral(oContent);
		}
	...
	CUnicodeParser.prototype.GetRowLiteral = function () {
		...
		return {
			type: oLiteralNames.rowLiteral[num],
			value: arrRow,
		};
	};
	CUnicodeParser.prototype.GetRowsLiteral = function () {
		...
		return {
			type: oLiteralNames.rowsLiteral[num],
			value: arrRows,
		};
	};
